...
<html>
	<head>
		<link rel='stylesheet' href='gpu_ud.css'></link>
		...
		<form action="#" method="POST" enctype='multipart/form-data'>
			<?php
				if(isset($_POST["UpdateCPU"]))
				{
					...
					$url="../upload/gpu/".$file_name;
					
					move_uploaded_file($tmp_name,$path);
					
					//with img $query="update gpu set series='$series', name='$name',compute_units='$compute_units', stream_processors='$stream_processors', texture_units='$texture_units', stock_freq='$stock_freq', boost_freq='$boost_freq', game_freq='$game_freq', transistor_size='$transistor_size', transistor_count='$transistor_count', output='$output',memory_type='$memory_type', VRAM='$vram', prize='$prize', in_stock='$in_stock', type='$hardware_type', img='$url' where no='$id'";
						...
						?>
							<div class='main_container'>
								...
								<div class='row'>
									...
									<div class='col4'>
										<div class='group'>
											<input type='text' name='in_stock' id='' class='input' required value="<?php echo $temp[16]; ?>">
											<span class='bar stockBar'></span>
											<label>Stock</label>
										</div>
										<div class='marginMakerNearStock'></div>
									</div>
								</div>
